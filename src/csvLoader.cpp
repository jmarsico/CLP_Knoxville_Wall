...
void CsvLoader::loadLightPoints(string filename){
    ...
    lightLocationCsv.loadFile(ofToDataPath(filename));
    ofLog() << "SIZE: " << ofToString(lightLocationCsv.data.size());
    for(int i = 0; i < lightLocationCsv.data.size(); i++)
    {
        //        ofLog() << "num: " << lightsCsv.data[i][0] << " x: " << lightsCsv.data[i][1] << " y: " << lightsCsv.data[i][2];
        
        ofVec2f lp;
        lp.set( ofToInt(lightLocationCsv.data[i][1]),  (-ofToInt(lightLocationCsv.data[i][2])));
